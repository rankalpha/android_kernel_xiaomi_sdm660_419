...
unsigned long schedutil_cpu_util(int cpu, unsigned long util_cfs, unsigned long max, enum schedutil_type type, struct task_struct *p) {
    unsigned long util, irq, dl_util;
    struct rq *rq = cpu_rq(cpu);

    /* Check if RT max frequency feature is enabled */
    if (!(sched_feat(SUGOV_RT_MAX_FREQ) && !uclamp_is_used() && type == FREQUENCY_UTIL && rt_rq_is_runnable(&rq->rt))) {
        /* Early check to see if IRQ/steal time saturates the CPU */
        irq = cpu_util_irq(rq);
        if (unlikely(irq >= max))
            return max;
        ...
        /* Compute DL utilization */
        dl_util = cpu_util_dl(rq);

        /* Check if utilization exceeds maximum */
        if (util + dl_util >= max) {
            /* Adjust for energy computation */
            if (type == ENERGY_UTIL)
                util += dl_util;

            /* Scale utilization by IRQ capacity */
            util = scale_irq_capacity(util, irq, max) + irq;

            /* Adjust for DL bandwidth */
            if (type == FREQUENCY_UTIL)
                util += cpu_bw_dl(rq);
